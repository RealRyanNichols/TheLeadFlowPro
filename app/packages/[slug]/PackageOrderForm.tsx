...
export default function PackageOrderForm({
  ...
  const savedLeadPayload = useRef<string | null>(null);
  ...
  const availableIntents = isCompanyOS
    ? INTENTS.filter(
        (option) => option.id === "map_first" || option.id === "questions",
      )
    : isWebsiteLaunch
      ? INTENTS.filter(
          (option) => option.id === "down_payment" || option.id === "questions",
        )
      : slug === "system-map"
        ? INTENTS.filter((option) => option.id !== "map_first")
        : INTENTS;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // A checkout retry reuses the saved request only while its details match.
      // Keep this in memory, rather than persisting contact details in the browser.
      if (savedLeadPayload.current !== leadPayload) {
        let res: Response;
        try {
          ...
          setError(
            "Could not reach the project intake. Check the connection and try again.",
          );
          return;
        }
        if (!res.ok) {
        ...
        savedLeadPayload.current = leadPayload;
        try {
          window.fbq?.("track", "Lead");
        } catch {
          // Optional analytics must not interrupt a saved request or payment.
        }
      }
      ...
      if (paysNow) {
        try {
          ...
          // Checkout still works when the analytics provider is unavailable.
        }
        ...
        try {
          ...
          // The saved inquiry remains available even if checkout cannot open.
        }
        setPayNotice(
          `Your request is saved, but checkout could not open. Payment has not been completed here. Retry secure checkout for ${fmt(payAmount)} below. If your details stay the same, retrying will not send another request.`,
        );
        return;
      }
      setSubmitted(true);
    ...
    return (
      <div className="rounded-[20px] border border-[var(--green-line)] bg-emerald-500/[0.06] p-9 text-center">
        ...
        <p className="mx-auto mt-3 max-w-xl text-[var(--text)]">
          ...
          The next step follows the written scope and payment schedule shown on
          this page.
        </p>
    ...
    <div className="rounded-[20px] border border-line bg-[var(--fill-2)] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.4)] sm:p-9">
          ...
          return (
            ...
            >
              <span className="flex items-center justify-between gap-2">
                <span className="text-[15px] font-bold text-[var(--heading)]">
                  {opt.label}
                </span>
                <span
      ...
      {intent === "down_payment" && !isWebsiteLaunch && (
        <div className="mt-4 rounded-xl border border-[var(--accent-line)] bg-sky-500/[0.06] p-5">
          ...
          <p className="mt-1 text-[12.5px] text-[var(--muted)]">
            Your call. Every dollar is credited in full toward the {packageName}{" "}
            build. Minimum {fmt(DEPOSIT_MIN)}.
          </p>
      ...
      {intent === "pay_full" && (
        <div className="mt-4 rounded-xl border border-[var(--accent-line)] bg-sky-500/[0.06] p-5">
          ...
          <p className="mt-1 text-[12.5px] text-[var(--muted)]">
            That covers the base scope. If the System Map scopes your build
            bigger, the difference is a separate approval. If it scopes smaller,
            the difference is credited or refunded. Every phase follows its
            written scope and milestone.
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="mt-7">
        ...
        {payNotice && (
          <p
            className="mt-4 rounded-xl border border-amber-600/30 bg-amber-50 p-4 text-sm text-stone-800"
            role="alert"
          >
            {payNotice}
          </p>
          ...
          {sending
            ...
            : paysNow
              ? Number.isFinite(payAmount)
                ? payNotice
                  ? `Retry Secure Checkout | ${fmt(payAmount)}`
