...
export default async function ClientWarRoom() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-10">
      <div className="overflow-hidden rounded-[30px] border border-[var(--line)] bg-[#ede6f3] p-6 text-[var(--heading)] shadow-[var(--lf-shadow)] sm:p-9">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--blue)]">Client War Room</p>
        <div className="mt-2 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div><h1 className="text-4xl font-black tracking-tight sm:text-5xl">{project.name}</h1><p className="mt-3 text-sm text-[var(--muted)]">{mission ? mission.name : "Delivery mission"} · {project.status.replaceAll("_", " ")}</p></div>
          {project.live_url && <a href={project.live_url} target="_blank" rel="noreferrer" className="rounded-xl bg-[var(--blue)] px-5 py-3 text-sm font-black text-white">Open live system</a>}
        </div>

        {mission && <div className="mt-7 rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-5">
          <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wide text-[var(--muted)]">{mission.target_label}</p><p className="mt-1 text-3xl font-black">{mission.current_value} / {mission.target_value} {mission.unit.replaceAll("_", " ")}</p></div><span className="text-2xl font-black text-[var(--blue)]">{missionProgress}%</span></div>
          <div className="mt-4 h-3 overflow-hidden rounded-full bg-[var(--fill-3)]"><div className="h-full rounded-full bg-gradient-to-r from-[#5135e5] to-[#957ddb]" style={{ width: `${missionProgress}%` }} /></div>
        </div>}
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        {metricCards.map(({ label, value, icon: Icon }) => (
          <div key={label} className="rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-4">
