...
export async function POST(request: Request) {
  try {
    ...
    if (lessonError || !lessons || lessons.length !== CONTENT_ENGINE_LESSONS.length) throw new Error("Published lesson count is not ready");
    const lessonIds = lessons.map((lesson) => lesson.id);
    const [progress, assignments, checks, finals, existing] = await Promise.all([
    ...
    const queryError = progress.error || assignments.error || checks.error || finals.error || existing.error;
    ...
    const ready = lessonIds.every((id) => completedLessons.has(id) && completedAssignments.has(id) && passedChecks.has(id)) && !!final;
    if (!ready) {
      return NextResponse.json({ error: "Finish all 12 lessons and assignments, pass every lesson check, and pass the final assessment first." }, { status: 409 });
    }

    const finalScore = Math.round((final.score / final.max_score) * 100);
    const credentialCode = `LFP-OA02-${crypto.randomBytes(4).toString("hex").toUpperCase()}`;
