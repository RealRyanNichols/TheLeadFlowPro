    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="card !p-4 text-center">
          <div className="text-3xl font-black text-white">{s.total_views}</div>
          <div className="text-xs uppercase tracking-wide text-slate-400">Page views</div>
        </div>
        <div className="card !p-4 text-center">
          <div className="text-3xl font-black text-flow-400">{s.unique_visitors}</div>
          <div className="text-xs uppercase tracking-wide text-slate-400">Visitors</div>
        </div>
        <div className="card !p-4 text-center">
        ...
        <div className="card !p-4 text-center">
          ...
          <div className="text-xs uppercase tracking-wide text-slate-400">Event signups</div>
        </div>
      </div>

      <div className="card mt-6">
        <h2 className="mb-4 font-bold text-white">Views by day</h2>
        <div className="flex h-32 items-end gap-1">
          {s.views_by_day.length === 0 && (
            <p className="text-sm text-slate-400">No traffic logged yet.</p>
          )}
          {s.views_by_day.map((d) => (
            <div key={d.day} className="group relative flex-1">
              ...
              />
              <span className="absolute -top-6 left-1/2 hidden -translate-x-1/2 rounded bg-line px-1.5 py-0.5 text-[10px] text-white group-hover:block">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-3">
        <div className="card">
          <h2 className="mb-3 font-bold text-white">Top pages</h2>
          <ul className="space-y-2 text-sm">
            ...
            {s.top_pages.length === 0 && <li className="text-slate-400">No data yet.</li>}
          </ul>
        </div>
        <div className="card">
          <h2 className="mb-3 font-bold text-white">Traffic sources (UTM)</h2>
          <ul className="space-y-2 text-sm">
            {s.top_sources.map((p) => (
              <li key={p.source} className="flex justify-between gap-2">
                <span className="truncate text-slate-300">{p.source}</span>
                <span className="font-bold text-flow-400">{p.views}</span>
              </li>
            ))}
            {s.top_sources.length === 0 && <li className="text-slate-400">No data yet.</li>}
          </ul>
        </div>
        <div className="card">
          <h2 className="mb-3 font-bold text-white">Leads by source</h2>
          <ul className="space-y-2 text-sm">
            ...
            {s.leads_by_source.length === 0 && <li className="text-slate-400">No data yet.</li>}
          </ul>
        </div>
      </div>
