    ...
    ? [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${tool.name} | A free tool from The LeadFlow Pro`,
        },
      ]
    : undefined;
  return {
    title:
      tool.seoTitle ||
      `${tool.name}: free ${TOOL_TYPES[tool.toolType].label.toLowerCase()}`,
    description:
      tool.seoDescription ||
...
export default async function ToolPage({
  ...
  return (
    <main className="cb-page">
      ...
      <section className="mx-auto mt-14 w-[min(1040px,100%-40px)]">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <article className="rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-5">
            ...
            <h3 className="mt-3 text-base font-black text-[var(--heading)]">
              Who it is for
            </h3>
          ...
          <ol className="mt-6 space-y-4">
            {tool.steps.map((s, i) => (
              <li key={i} className="tool-step">
                ...
                <span className="pt-1 text-[15px] leading-relaxed text-[var(--text)]">
                  {s}
                </span>
              </li>
            ))}
      ...
      {guides.length > 0 && (
        <section
          aria-labelledby="tool-guides-title"
          className="mx-auto mt-12 w-[min(1040px,100%-40px)]"
        >
          <p className="cb-eyebrow">Keep learning</p>
          <h2
            id="tool-guides-title"
            className="text-2xl font-black tracking-tight text-[var(--heading)] sm:text-3xl"
          >
            Go a little deeper.
          </h2>
          <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-[var(--muted)]">
            See how this tool fits a practical task, with a guide you can follow
            at your own pace.
          </p>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {guides.map((guide) => (
              <Link
                ...
                className="group flex flex-col rounded-2xl border border-[var(--accent-line)] bg-[var(--accent-tint)] p-6 transition hover:border-[var(--blue)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--blue)]"
              >
                <BookOpen
                  aria-hidden="true"
                  className="h-6 w-6 text-[var(--blue)]"
                />
                <h3 className="mt-4 text-lg font-black leading-snug text-[var(--heading)]">
                  {guide.title}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-[var(--muted)]">
                  {guide.description}
                </p>
                <span className="mt-auto flex items-center gap-2 pt-5 text-sm font-black text-[var(--blue)]">
                  Read the guide{" "}
                  <ArrowRight
                    aria-hidden="true"
        ...
        <section className="mx-auto mt-12 w-[min(1040px,100%-40px)]">
          ...
          <div className="mt-5 space-y-3">
            {tool.faqs.map((f, i) => (
              <details
                key={i}
                className="group rounded-xl border border-[var(--line)] bg-[var(--panel)] p-5"
              >
      ...
      <section className="mx-auto mt-12 w-[min(1040px,100%-40px)]">
        <div className="rounded-2xl border border-[var(--accent-line)] bg-[var(--accent-tint)] p-6 sm:p-8">
          ...
          <p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-[var(--text)]">
            ...
            problem costs, and now they are on your website instead of a
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-[var(--line)] bg-[var(--panel)] p-4">
              <h3 className="text-sm font-black text-[var(--heading)]">
                Works on
              </h3>
      ...
      <section className="mx-auto mt-14 w-[min(1040px,100%-40px)]">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <Link
            href="/tools"
            className="text-sm font-bold text-[var(--blue)] hover:text-[var(--heading)]"
          >
