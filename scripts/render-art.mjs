...
if (sheet) {
  ...
  const cells = slugs.map((s, i) => {
    ...
    return `<div style="position:absolute;left:${x}px;top:${y}px;width:${w}px;height:${h + label}px">
      <img src="file://${SCENES}/${s}.svg" width="${w}" height="${h}" style="display:block;background:#fff;border:1px solid #ccc;box-sizing:border-box">
  ...
  for (const slug of slugs) {
    ...
    await page.setContent(
      `<body style="margin:0"><img src="file://${src}" width="800" height="450" style="display:block;background:#fff"></body>`
    );
