...
/**
 * A paid checkout, through post_creator_record_purchase. Idempotent per
 * session. A new email gets an account created by this session; a later
 * checkout on an existing email applies the plan rules (one payment never
 * downgrades) and bumps access_epoch, which signs every device out.
 * `createdByThisSession` is what the claim route needs: only the checkout
 * that created the account may sign a browser in.
 */
export async function recordPurchase(
  db: Db,
  p: PurchaseRecord,
): Promise<{ account: Account; created: boolean; createdByThisSession: boolean }> {
 ...
 * the account, no browser has claimed it yet, and no later checkout has
 * bumped the epoch (a later checkout signs every device out, including one
 * that had not arrived yet). The guard trigger allows first_claimed_at to go
...
export async function setStatus(db: Db, email: string, status: AccountStatus): Promise<void> {
  const { error } = await db.from(ACCOUNTS).update({ status }).eq("email", email);
