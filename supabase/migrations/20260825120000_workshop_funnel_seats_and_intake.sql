...
--
-- The public site never writes to event_registrations directly. Everything the
-- browser can reach goes through the security-definer functions at the bottom
-- of this file, so RLS stays exactly as it was: events are readable only when
-- published, registrations stay admin-read-only, and no service-role key ever
-- has to reach the client.
--
-- Seat accounting rule: a seat is consumed only by a PAID registration.
-- 'pending' is a lead who started the form. 'cancelled', 'transferred', and
-- 'refunded' hand the seat back. 'attended' and 'no_show' still hold the seat
-- they paid for. 'overbooked' means money arrived after the room filled and
-- needs a human refund decision.
...
comment on column public.events.date_confirmed is
  'False until the instructor locks the date. The funnel shows the date as a working date and blocks publishing prompts until this is true.';
...
-- Legacy statuses map onto the paid-seat model: a bare registration is a lead
-- ('pending'), a confirmed one had already paid ('paid').
...
-- Public seat count. Returns nothing for an unpublished event, so a draft
-- funnel cannot even be probed for availability.
...
-- Starts a registration. Never holds a seat: the seat is claimed by payment.
-- Refuses unpublished events, closed registration, and a full room.
...
-- Called only by the Stripe webhook. Locks the event row so two simultaneous
-- payments cannot both take the last seat; money that lands after the room is
-- full is parked as 'overbooked' for a human refund decision instead of being
-- silently dropped.
...
begin
  ...
  -- Stripe retries the same event; a seat already claimed stays claimed.
...
-- Confirmation page lookup. Deliberately narrow: no email, no phone, no notes.
...
-- One bottleneck per attendee, editable until the room starts.
